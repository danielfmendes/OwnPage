...
export const FilterBar: FC<FilterBarProps> = ({ filters, filterManager, onChange, isMobile = false, refreshSignal = 0 }) => {
    ...
    const clearLoadedItems = useFilterStore((state) => state.clearLoadedItems);
    ...
    // Clear the cache whenever the parent signals a data refresh (skip on initial mount)
                        ...
                        <PopoverTrigger asChild>
                            <div className="relative">
                                <ButtonLoading
                                    ...
                                    className={`rounded-full text-sm flex items-center gap-2 ${(selectedValues[key]?.length ?? 0) > 0 ? "pr-7" : ""
                                        }`}
                                >
