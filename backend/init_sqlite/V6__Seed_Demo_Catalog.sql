...
-- NON-DESTRUCTIVE: only inserts catalog rows pointing at existing tables (is_managed = 0 → the
-- physical tables are never altered or dropped). bikes.model_id is recorded as a catalog reference.
--
-- Apply to prod D1 out of band (once):
...
INSERT INTO dwh_entities (project_id, name, display_name, physical_table, project_column, is_managed) VALUES
    (1, 'bike_models', 'Bike Models', 'bike_models', NULL,         0),
    (1, 'customers',   'Customers',   'customers',   'project_id', 0),
    (1, 'bikes',       'Bikes',       'bikes',       'project_id', 0);

-- bike_models
INSERT INTO dwh_columns (entity_id, name, display_name, data_type, position, is_system) SELECT id, 'id', 'ID', 'integer', 0, 1 FROM dwh_entities WHERE project_id = 1 AND name = 'bike_models';
...
-- customers
INSERT INTO dwh_columns (entity_id, name, display_name, data_type, position, is_system) SELECT id, 'id', 'ID', 'integer', 0, 1 FROM dwh_entities WHERE project_id = 1 AND name = 'customers';
...
INSERT INTO dwh_columns (entity_id, name, display_name, data_type, position, is_system) SELECT id, 'project_id', 'Project', 'integer', 6, 1 FROM dwh_entities WHERE project_id = 1 AND name = 'customers';

-- bikes (model_id is a catalog reference -> bike_models)
INSERT INTO dwh_columns (entity_id, name, display_name, data_type, position, is_system) SELECT id, 'id', 'ID', 'integer', 0, 1 FROM dwh_entities WHERE project_id = 1 AND name = 'bikes';
...
INSERT INTO dwh_columns (entity_id, name, display_name, data_type, position, is_system) SELECT id, 'project_id', 'Project', 'integer', 6, 1 FROM dwh_entities WHERE project_id = 1 AND name = 'bikes';
