...
-- (is_managed=0). customers/bikes already carry a project_id discriminator; bike_models is global.
-- Apply to prod D1 once after V5:
--   npx wrangler d1 execute db-prod --file=backend/init_sqlite/V6__Seed_Demo_Catalog.sql --remote
...
SELECT id, 'bike_models', 'Bike Models', 'bike_models', NULL,         0 FROM dwh_schemas WHERE name = 'Bike Demo';
INSERT INTO dwh_entities (schema_id, name, display_name, physical_table, project_column, is_managed)
SELECT id, 'customers',   'Customers',   'customers',   'project_id', 0 FROM dwh_schemas WHERE name = 'Bike Demo';
INSERT INTO dwh_entities (schema_id, name, display_name, physical_table, project_column, is_managed)
SELECT id, 'bikes',       'Bikes',       'bikes',       'project_id', 0 FROM dwh_schemas WHERE name = 'Bike Demo';

-- bike_models (global)
INSERT INTO dwh_columns (entity_id, name, display_name, data_type, position, is_system) SELECT id, 'id', 'ID', 'integer', 0, 1 FROM dwh_entities WHERE physical_table = 'bike_models';
...
INSERT INTO dwh_columns (entity_id, name, display_name, data_type, position, is_system) SELECT id, 'id', 'ID', 'integer', 0, 1 FROM dwh_entities WHERE physical_table = 'customers';
...
INSERT INTO dwh_columns (entity_id, name, display_name, data_type, position, is_system) SELECT id, 'project_id', 'Project', 'integer', 6, 1 FROM dwh_entities WHERE physical_table = 'customers';

-- bikes (model_id reference -> bike_models; project_id discriminator)
INSERT INTO dwh_columns (entity_id, name, display_name, data_type, position, is_system) SELECT id, 'id', 'ID', 'integer', 0, 1 FROM dwh_entities WHERE physical_table = 'bikes';
...
INSERT INTO dwh_columns (entity_id, name, display_name, data_type, position, is_system) SELECT id, 'project_id', 'Project', 'integer', 6, 1 FROM dwh_entities WHERE physical_table = 'bikes';
